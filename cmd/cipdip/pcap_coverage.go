...
func runPcapCoverage(flags *pcapCoverageFlags) error {
	...
	aggregate := &cipclient.PCAPCoverageReport{
		ServiceCounts:       make(map[uint8]int),
		ServiceResponseCt:   make(map[uint8]int),
		RequestEntries:      make(map[string]*cipclient.CIPCoverageEntry),
		EmbeddedEntries:     make(map[string]*cipclient.CIPCoverageEntry),
		UnknownServicePairs: make(map[string]int),
	}

	for _, pcapPath := range pcaps {
		report, err := cipclient.SummarizeCoverageFromPCAP(pcapPath)
		if err != nil {
...
func mergeCoverage(dst, src *cipclient.PCAPCoverageReport) {
...
func writeCoverageSummary(f *os.File, report *cipclient.PCAPCoverageReport) {
	...
	for _, key := range cipclient.SortedCoverageEntries(report.RequestEntries) {
	...
	if len(report.EmbeddedEntries) > 0 {
		...
		for _, key := range cipclient.SortedCoverageEntries(report.EmbeddedEntries) {
