...
var cipClassAliases = map[string]uint16{
	"identity_object":    CIPClassIdentityObject,
	"message_router":     CIPClassMessageRouter,
	"assembly":           CIPClassAssembly,
	"connection":         CIPClassConnection,
	"connection_manager": CIPClassConnectionManager,
	"file_object":        CIPClassFileObject,
	"event_log":          CIPClassEventLog,
	"motion_axis":        CIPClassMotionAxis,
	"time_sync":          CIPClassTimeSync,
	"modbus":             CIPClassModbus,
	"symbol_object":      CIPClassSymbolObject,
	"template_object":    CIPClassTemplateObject,
	"program_name":       CIPClassProgramName,
	"pccc_object":        CIPClassPCCCObject,
	"safety_supervisor":  CIPClassSafetySupervisor,
	"safety_validator":   CIPClassSafetyValidator,
	"tcp_ip_interface":   CIPClassTCPIPInterface,
	"ethernet_link":      CIPClassEthernetLink,
}

var cipClassAliasNames = map[uint16]string{
	CIPClassIdentityObject:    "identity_object",
	CIPClassMessageRouter:     "message_router",
	CIPClassAssembly:          "assembly",
	CIPClassConnection:        "connection",
	CIPClassConnectionManager: "connection_manager",
	CIPClassFileObject:        "file_object",
	CIPClassEventLog:          "event_log",
	CIPClassMotionAxis:        "motion_axis",
	CIPClassTimeSync:          "time_sync",
	CIPClassModbus:            "modbus",
	CIPClassSymbolObject:      "symbol_object",
	CIPClassTemplateObject:    "template_object",
	CIPClassProgramName:       "program_name",
	CIPClassPCCCObject:        "pccc_object",
	CIPClassSafetySupervisor:  "safety_supervisor",
	CIPClassSafetyValidator:   "safety_validator",
	CIPClassTCPIPInterface:    "tcp_ip_interface",
	CIPClassEthernetLink:      "ethernet_link",
}

// NormalizeAlias normalizes human-friendly names into alias keys.
...
// ParseServiceAlias returns the service code for a known alias string.
...
// ParseClassAlias returns the class code for a known alias string.
...
// ServiceAliasName returns the canonical alias for a service code.
func ServiceAliasName(code uint8) (string, bool) {
	name, ok := cipServiceAliasNames[code]
	return name, ok
}

// ClassAliasName returns the canonical alias for a class code.
func ClassAliasName(code uint16) (string, bool) {
	name, ok := cipClassAliasNames[code]
	return name, ok
}

// FormatServiceAlias returns a stable alias or hex fallback.
...
// FormatClassAlias returns a stable alias or hex fallback.
