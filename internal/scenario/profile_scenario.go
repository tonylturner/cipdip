...
func (s *ProfileScenario) Run(ctx context.Context, client cipclient.Client, cfg *config.Config, params ScenarioParams) error {
	// Check if we're running all roles (stress mode)
	if s.Role == "all" {
		return s.runAllRoles(ctx, client, cfg, params)
	}
...
// runAllRoles runs the profile scenario with all roles rotating (stress mode).
// This generates mixed traffic from all role perspectives to stress test the server.
func (s *ProfileScenario) runAllRoles(ctx context.Context, client cipclient.Client, cfg *config.Config, params ScenarioParams) error {
	roleNames := s.Profile.RoleNames()
	if len(roleNames) == 0 {
		return fmt.Errorf("profile has no roles defined")
	}

	params.Logger.Info("Starting profile scenario: %s (ALL ROLES stress mode)", s.Profile.Metadata.Name)
	params.Logger.Info("  Roles: %v", roleNames)

	// Create client engines for all roles
	type roleEngine struct {
		name   string
		engine *engine.ClientEngine
	}
	engines := make([]roleEngine, 0, len(roleNames))
	for _, roleName := range roleNames {
		eng, err := engine.NewClientEngine(s.Profile, roleName)
		if err != nil {
			return fmt.Errorf("create engine for role %s: %w", roleName, err)
		}
		engines = append(engines, roleEngine{name: roleName, engine: eng})
		params.Logger.Verbose("  Role %s: interval=%v, batch=%d, reads=%d, writes=%d",
			roleName, eng.PollInterval(), eng.BatchSize(),
			len(eng.GetAllReadTags()), len(eng.GetWritableTags()))
	}
	...
	// Use the shortest poll interval across all roles for the ticker
	minInterval := engines[0].engine.PollInterval()
	for _, re := range engines[1:] {
		if re.engine.PollInterval() < minInterval {
			minInterval = re.engine.PollInterval()
		}
	}

	// Calculate total operations for progress bar
	totalOps := int64(params.Duration / minInterval)
	if totalOps == 0 {
		totalOps = 1
	}
	progressBar := progress.NewProgressBar(totalOps, fmt.Sprintf("Profile: %s (ALL ROLES)", s.Profile.Metadata.Name))
	defer progressBar.Finish()
	...
	fmt.Printf("[CLIENT] Starting ALL ROLES stress scenario: %s\n", s.Profile.Metadata.Name)
	fmt.Printf("[CLIENT] Roles: %v\n", roleNames)
	fmt.Printf("[CLIENT] Tick interval: %v (shortest among all roles)\n", minInterval)
	fmt.Printf("[CLIENT] Will run for %v or until interrupted\n\n", params.Duration)

	// Main loop - rotate through roles on each tick
	...
	for {
		select {
		case <-ctx.Done():
			params.Logger.Info("ALL ROLES scenario completed (duration expired or cancelled)")
			fmt.Printf("[CLIENT] Scenario completed after %d polls\n", loopCount)
			return nil
		...
		case now := <-ticker.C:
			// Check deadline
			if now.After(deadline) {
				params.Logger.Info("ALL ROLES scenario completed: %d polls in %v", loopCount, time.Since(startTime))
				return nil
			}
			...
			// Get current role engine
			re := engines[currentRoleIdx]
			clientEngine := re.engine
			...
			if len(readBatch) > 1 && clientEngine.BatchSize() > 1 {
				// Use Multiple Service Packet
				if err := s.executeBatchedReadsForRole(ctx, client, readBatch, params, clientEngine, re.name); err != nil {
					params.Logger.Verbose("[%s] Batched read error: %v", re.name, err)
				}
			} else {
				// Serial reads
				for _, req := range readBatch {
					...
					select {
					case <-ctx.Done():
						params.Logger.Info("ALL ROLES scenario completed (deadline during serial reads)")
						return nil
					default:
					}
					s.executeSingleReadForRole(ctx, client, req, params, clientEngine, re.name)
				}
			}
			...
			for _, w := range writes {
				...
				select {
				case <-ctx.Done():
					params.Logger.Info("ALL ROLES scenario completed (deadline during writes)")
					return nil
				default:
				}
				s.executeWriteForRole(ctx, client, w, params, clientEngine, re.name)
			}

			loopCount++
			progressBar.Increment()

			// Rotate to next role for next tick
			currentRoleIdx = (currentRoleIdx + 1) % len(engines)
		}
	}
}

// executeBatchedReadsForRole performs a batched read with role context.
func (s *ProfileScenario) executeBatchedReadsForRole(ctx context.Context, client cipclient.Client, batch []engine.ReadRequest, params ScenarioParams, clientEngine *engine.ClientEngine, roleName string) error {
	// Build individual read requests
	requests := make([]protocol.CIPRequest, len(batch))
	for i, req := range batch {
		requests[i] = protocol.CIPRequest{
	...
	if err != nil {
		...
		params.Logger.LogOperation(
			"MSP_READ",
			fmt.Sprintf("[%s] batch(%d)", roleName, len(batch)),
			fmt.Sprintf("0x%02X", uint8(spec.CIPServiceMultipleService)),
			false,
			batchRTT,
			0,
	...
	if resp.Status == 0 && len(resp.Payload) > 0 {
		responses, err := cipclient.ParseMultipleServiceResponsePayload(resp.Payload, resp.Path)
		if err != nil {
			params.Logger.Verbose("[%s] Failed to parse MSP response: %v", roleName, err)
		} else {
			// Record metrics for each response
			perTagRTT := batchRTT / float64(len(responses))
			for i, subResp := range responses {
				if i < len(batch) {
					...
					metric := metrics.Metric{
						...
						TargetType:  params.TargetType,
						Operation:   metrics.OperationRead,
						TargetName:  fmt.Sprintf("[%s] %s", roleName, batch[i].TagName),
						ServiceCode: fmt.Sprintf("0x%02X", uint8(spec.CIPServiceGetAttributeSingle)),
						Success:     success,
						RTTMs:       perTagRTT,
						Status:      subResp.Status,
	...
	// Log batch success
	fmt.Printf("[CLIENT] [%s] MSP Read batch(%d): RTT=%.2fms status=0x%02X\n",
		roleName, len(batch), batchRTT, resp.Status)

	return nil
}

// executeSingleReadForRole performs a single tag read with role context.
func (s *ProfileScenario) executeSingleReadForRole(ctx context.Context, client cipclient.Client, req engine.ReadRequest, params ScenarioParams, clientEngine *engine.ClientEngine, roleName string) {
	path := s.resolveTagPath(req.TagName, clientEngine)
	...
	metric := metrics.Metric{
		...
		TargetType:  params.TargetType,
		Operation:   metrics.OperationRead,
		TargetName:  fmt.Sprintf("[%s] %s", roleName, req.TagName),
		ServiceCode: fmt.Sprintf("0x%02X", uint8(spec.CIPServiceGetAttributeSingle)),
		Success:     success,
		RTTMs:       rtt,
		Status:      resp.Status,
	...
	if success {
		fmt.Printf("[CLIENT] [%s] Read %s: status=0x%02X RTT=%.2fms\n", roleName, req.TagName, resp.Status, rtt)
	} else {
		fmt.Printf("[CLIENT] [%s] Read %s FAILED: %s\n", roleName, req.TagName, errorMsg)
	}
}

// executeWriteForRole performs a tag write with role context.
func (s *ProfileScenario) executeWriteForRole(ctx context.Context, client cipclient.Client, req engine.WriteRequest, params ScenarioParams, clientEngine *engine.ClientEngine, roleName string) {
	path := s.resolveTagPath(req.TagName, clientEngine)
	...
	if success {
		fmt.Printf("[CLIENT] [%s] Write %s=%v: status=0x%02X RTT=%.2fms\n", roleName, req.TagName, req.Value, resp.Status, rtt)
	} else {
		fmt.Printf("[CLIENT] [%s] Write %s FAILED: %s\n", roleName, req.TagName, errorMsg)
	}
}
